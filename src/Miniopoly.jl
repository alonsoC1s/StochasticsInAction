...
"""
mutable struct GameManager{N::Int}
    board::CyclicStaticVector{9,Square}
    players::CyclicStaticVector{N,Player}
    turn::UInt
    positions::Dict{Player,Int}
end

# function Base.show(io::IO, omp::OpinionModelProblem{T}) where {T}
function Base.show(io::IO, gm::GameManager{N}) where {N}
    println("""
    Miniopoly, at turn $(gm.turn) with $N players:
    At positions:
    $(gm.positions)
    """)
end
